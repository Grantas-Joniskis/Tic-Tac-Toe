...
var BotState;
(function (BotState) {
    BotState[BotState["FIRST"] = 0] = "FIRST";
    BotState[BotState["SECOND"] = 1] = "SECOND";
    BotState[BotState["THIRD"] = 2] = "THIRD";
})(BotState || (BotState = {}));
let botState;
function makeMove() {
    if (Game.getGameState() === State.BOT) {
        setTimeout(() => {
            if (getBotState() === BotState.FIRST)
                isFirstMove();
            else if (getBotState() === BotState.SECOND)
                isSecondMove();
            else if (getBotState() === BotState.THIRD)
                isThirdMove();
...
function moveToSquare(squareId) {
